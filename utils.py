...
def generate_sample_images_pix2pix(train_ds, val_ds, gen, epoch, EPOCHS, save_folder, QUIET_PLOT=True, log_wandb=True):
    ...
    for train_input, train_target in train_ds.take(1):
        filename_train = f"train_epoch_{str(epoch).zfill(len(str(EPOCHS)))}.jpg"
    ...
    for val_input, val_target in val_ds.take(1):
        filename_val = f"val_epoch_{str(epoch).zfill(len(str(EPOCHS)))}.jpg"
...
def cycle_test(dataset, gen_fwd, gen_bkw, ncycles, npictures, folder, start='A', QUIET_PLOT=True):
    ...
    for image in dataset.shuffle(npictures, seed=42).take(npictures):
        ...
        print(f"{start} -> {end} ({i})")
        filename = f"{str(i).zfill(len(str(npictures)))}_{fromto}_original.jpg"
        ...
        for c in range(ncycles):
            ...
            # Avança (FWD)
            filename = f"{str(i).zfill(len(str(npictures)))}_{fromto}_FwdClass{end}_cycle{str(c + 1).zfill(len(str(npictures)))}.jpg"
            image = gen_fwd(image)
            if not QUIET_PLOT:
                plt.figure()
                plt.imshow(image[0] * 0.5 + 0.5)
            ...
            # Retorna (BKW)
            filename = f"{str(i).zfill(len(str(npictures)))}_{fromto}_BkwClass{end}_cycle{str(c + 1).zfill(len(str(npictures)))}.jpg"
            image = gen_bkw(image)
            if not QUIET_PLOT:
                plt.figure()
                plt.imshow(image[0] * 0.5 + 0.5)
...
class ProjectError(Exception):
    def __init__(self, project):
        print(f"O projeto {project} não está definido")


class ProjectMismatch(Exception):
    def __init__(self, project, net_type):
        print(f"O projeto {project} não pode ser usado com a rede {net_type}")
...
class ArchitectureError(Exception):
    def __init__(self, net_type):
        print(f"A rede {net_type} não está definida")


class DatasetError(Exception):
    def __init__(self, net_type, dataset_folder):
        print(f"O dataset em {dataset_folder} não é compatível com a rede {net_type}")
